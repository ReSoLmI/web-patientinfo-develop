<template>
  <div>
    ...
    <div ms-controller="outpatient_view">
      <div class="am-page-lside-second" style="left: 0;">
        <div class="lside-info" v-show="patient.name" style="display:none;">
          ...
          <div class="more-cont">
            ...
            <div style="display: none;">
              <p class="p-tooltip">
                <span class="am-txt-muted am-mr-10">入院时间:</span>
                {{patient.admissionTime | date('yyyy-MM-dd hh:mm:ss')}}
              </p>
              <p class="p-tooltip">
                <span class="am-txt-muted am-mr-10">出院时间:</span>
                {{patient.dischargeTime | date('yyyy-MM-dd hh:mm:ss')}}
              </p>
              <p class="p-tooltip">
                <span class="am-txt-muted am-mr-10">联系电话:</span>
                {{patient.mobileNo||'--'}}
              </p>
            </div>
          </div>
        ...
        <ul class="lside-secondary-menu clearfix" v-show="menus.length>0" style="display:none;">
          <li
            v-for="(first, index) in menus"
            :key="index"
            :class="{'li-angle':first.subList.length>0}"
            @click="showSonMenu()"
          >
            <!-- 一级菜单 -->
            <a
              class="showTip bigLabel"
              :class="{active:first.menuId==FMenuId&&!SMenuId}"
              :data-title="first.menuName"
              @click="goToPageF(first)"
            >
              {{first.menuName}}
              <i class="angle"></i>
            </a>
            ...
            <ul v-if="first.subList&&first.subList.length>0" :id="'son2_' + first.menuId">
              <li
                v-for="(second, index1) in first.subList"
                :key="index1"
                @click="showPage()"
                class="son"
              >
                <a
                  class="showTip"
                  :class="{active:SMenuId==second.menuId&&!TMenuId,on:SMenuId==second.menuId&&TMenuId}"
                  :data-title="second.menuName"
                  @click="goToPageS(first,second)"
                >{{second.menuName}}</a>
                <!-- 三级菜单 -->
                <ul
                  class="ul-third"
                  style="display: block;"
                  v-if="second.subList.length>0"
                  :id="'son3_'+second.menuId"
                >
                  <li
                    v-for="(third, index2) in second.subList"
                    :key="index2"
                    @click="showPage()"
                    class="son"
                  >
                    <a
                      @click="goToPageT(first,second,third)"
                      class="showTip bigLabel"
                      :data-title="third.menuName"
                      :class="{active:TMenuId==third.menuId}"
                    >{{third.menuName}}</a>
                  </li>
                </ul>
              </li>
      ...
      <div class="am-page-rcont" style="left: 170px;">
        <div v-show="loading_iframe==1" style="height:100%;overflow: hidden;display: none">
          <iframe
            :src="pageUrl"
  ...
  methods: {
    ...
    getPatientInfo: function () {
      ...
      this.common.queryPatientInfo({
        data: {
          status: 'out',
          patIndexNo: that.common.getUrlParam('patIndexNo')
        },
        callback: (data) => {
          if (data.length > 0) {
            that.patient = data[0]
          } else {
            that.common.tip_msg('未找到相关出院患者信息！', null, function () {
              // avalon.vmodels.root.closeTab(
              //   'patient_out',
              //   this.common.getUrlParam('patIndexNo')
              // )
              window.location.href =
                '#!/outpatient/html/patients_list?wardCode=' +
                that.common.getUrlParam('wardCode')
            })
          }
        }
      })
    ...
    getPageUrl: function () {
      ...
      // console.log(this.FMenuId)
      // console.log(this.SMenuId)
      // console.log(this.TMenuId)

      var that = this
      this.queryPatientInfo((data) => {
        var dataInfo = {}
        if (data.length > 0) {
          dataInfo = data[0]
          // 获取科室数据源 科室数据永远取当前科室数据
          var deList = JSON.parse(that.common.getLocalStorage('PAT_deList')) || []
          if (deList && deList.length > 0) {
            for (var i = 0; i < deList.length; i++) {
              if (deList[i].wardCode === that.common.getUrlParam('wardCode')) {
        ...
        var urlInfo = this.common.getPageUrlNew(
          this.menus,
          this.FMenuId,
          this.SMenuId,
          this.TMenuId,
    ...
    queryPatientMenuNew: function () {
      var param = {
        ...
        menuType: 'pc',
        topMenuCode: 'outpatientV2', // this.common.getAppCode(),
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].subList.length > 0) {
              ...
              for (var m = 0; m < data[i].subList.length; m++) {
                data[i].subList[m].subList = data[i].subList[m].subList
                  ? data[i].subList[m].subList
                  : []
    ...
    goToPageF: function (item) {
      if (item && item.subList.length === 0 && item.menuUrl) {
        var href =
          '#/discharged/patientview?wardCode=' +
          this.common.getUrlParam('wardCode') +
          '&patIndexNo=' +
          this.common.getUrlParam('patIndexNo') +
          '&FMenuId=' +
          item.menuId
